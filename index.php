...
$result = $db->prepare("SELECT sum(amount) FROM payment WHERE    date='$date'  ");
		...
		date_default_timezone_set("Asia/Colombo");
		$date=date("Y-m-d");
		$result = $db->prepare("SELECT count(transaction_id) FROM payment WHERE  date='$date' ORDER by transaction_id DESC ");
				$result->bindParam(':userid', $date);
                $result->execute();
                for($i=0; $row = $result->fetch(); $i++){
...
<?php if($_SESSION['SESS_FIRST_NAME']=="NED"){ ?>
...
<span class="label label-<?php echo $row1['color']; ?> pull-right"><?php echo $row1['name']; ?></span>
<?php } ?>
</a>
